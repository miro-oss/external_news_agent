 ...
 * 대신 성공과 실패를 {@link FeedFetchResult}로 구분해서 돌려준다 — 빈 목록만 주면 호출부가
...
public class FeedClient {
    ...
    public FeedFetchResult fetch(String feedUrl, String language) {
        String body;
        try {
            body = restClient.get()
                    .uri(feedUrl)
        ...
        try {
            return FeedFetchResult.ok(FeedParser.parse(body, language));
        } catch (FeedParseException e) {
            log.warn("피드를 읽지 못했다. HTML 페이지를 FEED로 등록했을 수 있다. url={} error={}",
                    feedUrl, e.getMessage());
            return FeedFetchResult.unreadable(e.getMessage());
        }
    }
     ...
     */
    private FeedFetchResult failureOf(String feedUrl, RestClientException exception) {
        if (exception instanceof RestClientResponseException response) {
            ...
            if (retryable) {
                ...
                return FeedFetchResult.rateLimited("피드 응답 " + response.getStatusCode());
            }

            log.warn("피드 요청이 거부됐다. 재시도해도 같은 응답이라 여기서 멈춘다. status={} url={}",
                    response.getStatusCode(), feedUrl);
            return FeedFetchResult.unreadable("피드 응답 " + response.getStatusCode());
        }

        log.warn("피드 호출에 실패했다. url={} error={}", feedUrl, exception.getMessage());
        return FeedFetchResult.unreadable("피드 호출 실패: " + exception.getMessage());
    }
}
