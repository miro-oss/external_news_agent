...
class ArticleBodyCleanerNavigationTest {
    ...
    void preservesAmbiguousInlineProseWithoutAnExplicitBoundary(String body) {
        assertEquals(body, ArticleBodyCleaner.withoutLeadingNavigation(body));
    }

    @Test
    void removesOnlyTheDelimitedMenuBeforeAnUnseenCategoryInProse() {
        assertEquals("국제 정치 관계가 악화됐다.", ArticleBodyCleaner.withoutLeadingNavigation(
                "최신뉴스 | 정치 | 경제 | 사회 | 국제 정치 관계가 악화됐다."));
        assertEquals("국제·정치 관계가 악화됐다.", ArticleBodyCleaner.withoutLeadingNavigation(
                "최신뉴스 | 정치 | 경제 | 사회 | 국제·정치 관계가 악화됐다."));
    }
    ...
    @Test
    void preservesARepeatedHeadingAfterTheMenu() {
        String prose = "경제\n\n경제 전망은 밝다.";
    ...
    @Test
    void preservesAnUnconfirmedMenuRestartAsAHeading() {
        assertEquals("최신뉴스\n경제 전망은 밝다.", ArticleBodyCleaner.withoutLeadingNavigation(
                "최신뉴스\n정치\n경제\n사회\n최신뉴스\n경제 전망은 밝다."));
    }
    ...
    void removesReportedLeadingNewsMenu(String separator) {
        ...
        assertEquals(STORY, ArticleBodyCleaner.withoutLeadingNavigation(menu + "\n\n" + STORY));
    }
    ...
    void doesNotRemoveAHeadingAfterTheMenuWhenItBeginsARealSentence() {
        ...
        assertEquals(prose, ArticleBodyCleaner.withoutLeadingNavigation("최신뉴스\n정치\n사회\n국제\n" + prose));
        String flattened = "최신뉴스 정치 경제 사회 생활문화 스포츠 국제 날씨 " + prose;
        assertEquals(flattened, ArticleBodyCleaner.withoutLeadingNavigation(flattened));
    }
