...
전 P2-9 가동 이력이다. run마다 배포 commit을 기록하지 않아 당시 실행 바이너리의 revision까지
입증하지는 못한다. **#147 머지 후 실행만 요구하는 더 엄격한 배포 검증에는 이 네 run을
그대로 합격 표본으로 쓰지 않는다.** 이번 결과는 현재 main 규칙의 오프라인 재생이다.
...
사람 검수, 새 배포 후 관측 표본 및 새로운 holdout을 확보한 뒤 최종 판단한다.
