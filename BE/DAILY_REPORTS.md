...
최초 구현 `132b20a`의 실측이다. PR 리뷰 반영 후에는 유료 호출을 반복하지 않고 회귀 테스트로 검증했다.
...
- DAILY는 유효한 이슈 단위로만 집계한다. 이슈 멤버십 없는 레거시 finding, 병합으로 비워진 이슈,
  기사와 이슈의 주제가 다른 멤버십은 제외한다. RUN의 레거시 finding 유지 규칙과 의도적으로 다르다.
- `collected`는 RUN처럼 실행 item의 `scannedCount` 합계이며 item이 없으면 run의 값을 쓴다.
  같은 기사 재관측도 스캔 수에 포함한다. 접근 제한·실패는 집계일에 관측한 고유 기사 기준이다.
  임시 분석·근거 부족 제외 수는 이슈별 최신 분석에서 상위 N개 제한 전에 센다. 복구 시 포함 finding은
  저장된 ID를 유지하고 수집 제약 수치만 현재 저장 데이터로 재집계한다.
...
- 24시간 이상 PENDING/RUNNING인 실행은 집계일·건수·7일 창 밖 여부를 WARN으로 남긴다
  (오래된 날짜부터 최대 20개). 실행 상태를 임의로 종료하지 않는다. 7일 밖 날짜는 자동 보충되지 않으므로
  운영자가 중단 실행을 복구한 뒤 `DailyReportCreationService.generate(date)`로 명시적으로 생성한다.
- 짧은 DB 잠금으로 생성 소유자를 예약하고 HTTP 호출 전에 커밋한다. 30분 이상 남은 PENDING은
  저장한 finding ID로 복구하며 불확실한 LLM 호출을 반복하지 않는다. 완료된 RUN·DAILY는 덮어쓰지 않는다.
- DAILY 제목과 fallback 기간 문구는 생성기가 직접 만든다. 생성된 본문 전체에서 제목이나 인용문을
  찾아 치환하지 않는다. 조회 목록은 DB 집계만 읽고, 상세의 여러 실행 조사 이력은 한 번에 조회한다.
...
PR #157 리뷰 반영 후 검증: 일반 백엔드 729개 통과·조건부 110개 제외, Oracle 보고서 통합 3개 통과,
Agent 234개·Ruff·골든 replay 통과, FE 빌드·린트 통과. 브라우저에서 같은 기사가 다른 실행의
finding에도 존재하는 경우와 교차 출처 기사 선택 모두 원래 카드의 runId를 전송하는지 확인했다.
DAILY의 날짜가 누락된 테스트 응답도 저장된 분석을 유지했으며, UI 클래스와 CSS는 변경하지 않았다.
