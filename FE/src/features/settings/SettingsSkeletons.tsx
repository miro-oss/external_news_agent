    ...
    <SkeletonRegion label="수집 주제를 불러오는 중">
      ...
      <div className="table-scroll">
        <table className="topic-table topic-management-table settings-skeleton-topic-table">
          ...
          <thead><tr>{['주제', '검색 키워드', '지난주 대비 증가', '연관 키워드', '수집 일정', '보고서 알림', '관리']
            ...
            {Array.from({ length: 3 }, (_, row) => (
              <tr key={row}>
                {Array.from({ length: 7 }, (_, column) => (
                  <td key={column}>
                    {column >= 5 ? <div className="settings-skeleton-management-actions">
                      <Skeleton height="1.875rem" />{column === 6 && <Skeleton height="1.875rem" />}
                    </div> : <SkeletonText lines={column === 4 ? 2 : 1} />}
                  </td>
                ))}
              </tr>
            ))}
...
export function LlmUsageSummarySkeleton() {
  return (
    <SkeletonRegion label="오늘 OpenAI 사용량을 불러오는 중" contentClassName="run-usage-summary">
