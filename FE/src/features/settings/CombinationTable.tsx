import { useCombinations, useTopics } from '../../api/queries'
...
export function CombinationTable() {
  ...
  if (combinations.isPending || topics.isPending) {
    return <p className="muted">불러오는 중…</p>
  }

  const error = combinations.error ?? topics.error
  if (error || !combinations.data || !topics.data) {
    const reason = error instanceof ApiError ? `${error.message} (${error.code})` : '목록을 불러오지 못했습니다.'
    return <p className="error">{reason}</p>
  }

  if (combinations.data.content.length === 0) {
    return (
      <p className="muted">
  ...
  const topicById = new Map(topics.data.content.map((topic) => [topic.id, topic]))

  return (
    /*
      열 구성은 #76을 바탕으로 키워드 3종을 더한다 — 수집 건수는 시스템이 관리하게 되어 화면에서
      뺐고, 주기는 분 단위 숫자 대신 formatInterval이 읽을 수 있는 말로 바꾼다.
    ...
    */
    <div className="table-scroll">
      <table>
        ...
        <tbody>
          {combinations.data.content.map((row) => {
            ...
            return (
              <tr key={`${row.topicId}-${row.sourceId}`}>
                ...
                <td>{row.queryText ?? '—'}</td>
                <td>{topic ? formatKeywords(topic.requiredKeywords) : '—'}</td>
                <td>{topic ? formatKeywords(topic.optionalKeywords) : '—'}</td>
                <td>{topic ? formatKeywords(topic.excludedKeywords) : '—'}</td>
