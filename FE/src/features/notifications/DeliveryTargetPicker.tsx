...
import { DeliveryTargetSkeleton } from './NotificationSkeletons'
...
export function DeliveryTargetPicker({ value, onChange, disabled = false }: { value: DeliveryTargets; onChange: (value: DeliveryTargets) => void; disabled?: boolean }) {
  ...
  if (pending) return <DeliveryTargetSkeleton />
  ...
  return <div className="delivery-target-picker">
    ...
    <fieldset className="delivery-channel-options" disabled={disabled}><legend>전달 방식</legend><div className="delivery-choice-list">
      {activeChannels.map((channel) => <label key={channel.id}><input type="checkbox" checked={value.channelIds.includes(channel.id)} onChange={() => toggle('channelIds', channel.id)} /><span className="delivery-choice-name">{channel.channelType === 'EMAIL' ? '이메일' : '텔레그램'}</span></label>)}
    ...
    <fieldset disabled={disabled}><legend>수신 그룹</legend><div className="delivery-choice-list">
      {activeGroups.map((group) => <label key={group.id}><input type="checkbox" checked={value.groupIds.includes(group.id)} onChange={() => toggle('groupIds', group.id)} /><span className="delivery-choice-name">{group.name}</span><span className="muted delivery-choice-count">{group.activeMemberCount}명</span></label>)}
    ...
    <fieldset disabled={disabled}><legend>개별 수신자</legend><div className="delivery-choice-list">
      {activeRecipients.map((recipient) => <label key={recipient.id}><input type="checkbox" checked={value.recipientIds.includes(recipient.id)} onChange={() => toggle('recipientIds', recipient.id)} /><span className="delivery-choice-name">{recipient.name}</span></label>)}
