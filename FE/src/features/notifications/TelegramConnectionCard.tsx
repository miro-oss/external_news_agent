...
export function TelegramConnectionCard({ recipientId, recipientName }: { recipientId: number; recipientName: string }) {
  ...
  async function copyLink() {
    ...
    try {
      ...
      window.clearTimeout(copyTimer.current)
      setCopied(true)
      copyTimer.current = window.setTimeout(() => setCopied(false), FEEDBACK_DURATION_MS)
    } catch {
      window.clearTimeout(copyTimer.current)
      setCopied(false)
      setCopyError(true)
  ...
  return <div className="telegram-connection" aria-label={`${recipientName} 텔레그램 연결`}>
    ...
    {connection.isPending ? <TelegramConnectionSkeleton />
      ...
      : <>
        ...
        {!waiting && <button type="button" className="primary-button" disabled={link.isPending || disconnect.isPending}
          onClick={() => { resetCopyFeedback(); link.mutate() }}>{link.isPending ? '링크 준비 중…' : '연결 링크 만들기'}</button>}
        {waiting && !link.data && <div className="telegram-link-actions">
          ...
          <div className="telegram-link-buttons">
            ...
            <button type="button" className="text-button" disabled={link.isPending || disconnect.isPending} onClick={() => { resetCopyFeedback(); link.mutate() }}>{link.isPending ? '링크 준비 중…' : '링크 다시 만들기'}</button>
          </div>
        </div>}
      </>}
