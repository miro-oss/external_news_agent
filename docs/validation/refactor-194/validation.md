...
- 사이드바 바탕은 선택색과 같은 계열의 아주 옅은 블루로 변경했다. 기존 `--primary-weak`를 흰색과 45% 혼합하고 hover는 70%, 선택 행은 기존 100% 색상으로 구분한다. CUA에서 기본 배경·선택 면·본문의 색상 조화를 확인하고 FE production build를 다시 통과했다.
- 실행별·일일 통합은 배경 없는 텍스트 탭으로 정리했다. 선택한 탭은 진한 글자와 글자 폭에 맞춘 3px 파란 밑줄로 표시하며, 보고서 바깥의 기존 위치와 간격을 유지한다. 기존 Segmented 이동 모션을 유지했고 CUA에서 일일 통합 전환 시 보고서 목록·본문 변경, 실행별 복귀, 모바일 가로 넘침 없음을 확인했다. 변경 후 FE production build가 통과했다.
...
- 상단 메뉴 선택 배경은 `--line-soft`(#f2f4f6), 글자는 `--fg`로 변경했다. CUA에서 알림 관리·리포트·수집 설정 전환과 회색 선택 면 위치를 확인했다. 모바일 메뉴 트랙은 투명하게 바꿔 회색 선택 면이 구분된다. 기존 선택 이동·크기 애니메이션은 유지했다.
