...
- 실행별·일일 통합 두 버튼에만 블루 트랙과 작은 둥근 모서리, 흰 선택 면·파란 글자를 적용했다. 보고서 바깥의 기존 위치와 간격을 유지하며 가로로 감싸는 도구 영역이나 외곽 박스는 추가하지 않았다. 기존 Segmented 이동 모션을 유지했고 CUA에서 일일 통합 전환 시 보고서 목록·본문 변경, 실행별 복귀, 모바일 가로 넘침 없음을 확인했다.
