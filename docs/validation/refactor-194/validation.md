...
- 데스크톱 목록과 본문을 같은 둥근 카드 안에 배치했다. 선택 항목의 흰 배경이 위아래 오목한 곡선을 거쳐 본문으로 직접 이어진다. 연결선·위치 측정 코드·상단 색상 그라데이션 없이 CSS로 구현했다. 기존 테마 색상·모서리·그림자·모션 토큰을 사용한다.
- 본문 전체의 좌우 여백은 화면 폭에 따라 32~56px로 늘렸다(1440px에서 약 50px, 920px에서 약 32px). 제목·통계·본문 카드가 같은 기준선으로 정렬된다. 제목 위 40px 여백은 유지했다.
- CUA에서 1440px·920px·390px을 확인했다. 데스크톱의 선택 항목 오른쪽과 본문 왼쪽 좌표 차이는 0px이며, 다른 항목과 목록 끝 항목을 선택했을 때도 본문 제목과 선택 면이 일치한다. 목록 스크롤바만 왼쪽에 배치하고 내용의 읽기 방향은 유지했다.
- 모바일에서는 기존 가로 목록과 개별 카드를 유지하고 연결 곡선을 숨긴다. 스크롤 방향과 24px 본문 여백도 복원했다. 세 화면 모두 페이지 가로 넘침이 없다.
- 목록 폭은 230~280px로 제한하고 제목(최대 두 줄)과 생성 시간만 표시한다. 목록의 반복 분석 건수·빨간 민감도 숫자·범위 문구를 제거했다. 전체 제목은 접근 가능한 텍스트와 title 속성에 보존하며 본문 통계와 이슈 정보는 유지한다. 비선택 제목의 강조를 낮추고 곡선은 8px로 줄였다. 데스크톱 스크롤바는 목록에 마우스를 올리거나 키보드 초점이 들어왔을 때 표시한다.
