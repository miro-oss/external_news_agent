...
- 50개 수동 이슈 라벨을 요약형·부분 어휘형·47시간 후속형 제목 4건으로 확장해 200건을 구성했다.
- 모든 골든 기사에 서로 다른 FULLTEXT 원고를 넣어 Java SimHash 경로도 함께 통과시켰다.
...
|---|---:|---:|---:|---:|---:|---:|
| Calibration 선택 | 0.50 | 48h | 1.000 | 0.988 | 0.994 | 0.998 |
| Holdout 재검증 | 0.50 | 48h | 1.000 | 1.000 | 1.000 | 1.000 |
| 기존 초기값 | 0.60 | 48h | 1.000 | 0.600 | 0.746 | 0.930 |
| char_wb TF-IDF 비교군 | 0.50 | — | 1.000 | 1.000 | 1.000 | 1.000 |

## 결정

- `news.clustering.title-jaccard-threshold=0.50`
- `news.clustering.entity-time-window=48h`
- 홀드아웃 precision이 완료 기준 `0.90`을 통과하고 recall도 `0.85` 이상이므로 Oracle VECTOR 임베딩은 도입하지 않는다.
- TF-IDF도 calibration에서 0.50을 선택한 뒤 holdout에 고정 적용했다. 홀드아웃 결과가 동률이어서 런타임 서비스 홉과 별도 IDF 정본을 추가할 이득이 없다.
