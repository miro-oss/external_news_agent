...
이미 실행 중인 Uvicorn과 IntelliJ 백엔드는 자동 reload 설정이 아니므로 재시작해야 적용된다.
이 문서의 검증은 코드·계약 회귀 검증이며, 수정 후 실제 Gemini 수집 성공을 뜻하지 않는다.
기존 독립 검증 자료와 실행 결과를 바꾸거나 추가 수집을 실행하지 않았다.
